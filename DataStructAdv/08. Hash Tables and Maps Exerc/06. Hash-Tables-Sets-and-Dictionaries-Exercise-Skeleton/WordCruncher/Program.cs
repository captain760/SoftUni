...
// *** Recursive solution *** (two recursive methods)
...
namespace WordCruncher
{
    ...
    class Cruncher
    {
        ...
        public IEnumerable<string> GetSyllablePaths()
        {
            ...
            return allPaths.Select(x => string.Join(" ", x)).Distinct();
                 //new HashSet<string>(allPaths.Select(x => string.Join(" ", x)));   Generate unique paths only by .Distict() or HashSet
        }
