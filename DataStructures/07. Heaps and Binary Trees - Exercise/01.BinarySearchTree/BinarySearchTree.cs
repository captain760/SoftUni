    ...
    public class BinarySearchTree<T> : IBinarySearchTree<T> where T : IComparable
    {
        ...
        private Node Delete(Node node, T element)
        {
            ...
            if (element.CompareTo(node.Value)==0)
            {
                ...
                {
                    node.Value = this.FindTheMostLeftChild(node.Right).Value;
        ...
        private Node FindTheMostLeftChild(Node node)
        {
        ...
        public IEnumerable<T> Range(T startRange, T endRange)
        {
            ...
            this.Range(this.root, startRange, endRange, resultQueue);
            return resultQueue;
        }

        private void Range(Node node, T startRange, T endRange, Queue<T> resultQueue)
        {
            if (node==null)
            {
                return;
            ...
            if (nodeInLowerRange)
            {
                this.Range(node.Left, startRange, endRange, resultQueue);
            }
            if (startRange.CompareTo(node.Value)<=0 && endRange.CompareTo(node.Value)>=0)
            {
                resultQueue.Enqueue(node.Value);
            }
            if (nodeInUpperRange)
            {
                this.Range(node.Right, startRange, endRange, resultQueue);
            }
        }
